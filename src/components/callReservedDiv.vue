<template>
  <div
    v-if="state.isShow"
    :class="['sc-bar', scheduleDetail.isLable ? 'isLable' : (state.isMe ? 'isMe' : 'notMe')]"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const setLeftPosition = () => {
      ...
      let shiftCnt = leftDiff / props.unit;
      let shiftCnt1 = parseInt(leftDiff / props.unit);
    ...
    const setWidth = () => {
      ...
      let widthCnt = rightDiff / props.unit;
      let widthCnt1 = parseInt(rightDiff / props.unit);
      ...
      // max width set by vinothcl
      let rightDiffNew = getMinutesDiff(
        new Date(props.startText),
        new Date(props.minDate + ' 23:50')
      );
      let widthCntNew = rightDiffNew / props.unit;
      let maxWidth = props.unitWidth * widthCntNew + widthCntNew * props.borderWidth;
      state.styleObject.maxWidth = 9 + maxWidth + "px";
